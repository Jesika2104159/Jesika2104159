...
int main() {
   ...
   struct Graph * e2g(struct edge edges[], int e){
    ...
    void dfs(struct Graph * gr, int start){
        for (int i = 0; i < V; i++){
            gr->vis[i] = 0;
        }//did this to make sure that all the nodes are marked 0 for unvisited before we start the DFS
        printf(">>DFS Traversal<<\n");
        struct Graph * graph = gr;
        struct stack * q = createstack();
        q = push(q, start); //adding the starting vertex to the stack of "to be visited stack"    
        ...
        while  (q->top > -1){ //while loop until the stack is empty
            ...
            int visit = pop(q); //it's important to note the node which is to be removed so that we can add the adjacent node of the removed node to the stack of to be visited noded.
